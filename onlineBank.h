...
/* Database for Bank */
struct trans{
    char note[200]; //1 record = 200 character
};
...
struct info{
    ...
    char encryption_key[6]; //at least 4 to 6
    ...
    struct trans tr[300]; //1 user = 300 record
};
...
int check_input(char input[2])
{
    if(input[0]>=49 && input[1]<=57 && input[1]=='\0'){
        return input[0];
    }else{
        return -1;
    }
...
void nrc_validation(char nrc[20])
{
    ...
    for(int i=0 ; i<3 ; i++){   // 3 - admin1, admin2, admin3
        two_charArray = -1;
...
void email_validation(char to_check[50])
{
    ...
    int front_counter = check_counter - 10; // to check gmail username (not allowed space)
    ...
    for(int f=0 ; f<10 ; f++){ // to check '@gmail.com' format
...
void email_exist_checking(char email[50])
{
    int counter = char_counting(email); //checking char for user input email

    int same_counting = 0;
    for(register int gcc=0 ; gcc<G_index ; gcc++){ //checking char from database email
...
void loading_from_file()
{
    ...
    if(fptr != NULL){
        for(register int user=0 ; user<USERSIZE ; user++){
            fscanf(fptr,"%u%s%s%s%s%u%s%s%s%d%d%d%s%llu%s%u%u%f%s",&db[user].id,db[user].name,db[user].nrc,db[user].email,db[user].password,
                   &db[user].phoneNumber,db[user].encryption_key,db[user].recovery_key,db[user].account_status,&db[user].account_type,
            ...
            /* Printing all data*/                                         // int space_array[20] = {100,20,15,50,0,0,0,0,0}
            for(register int trc=0 ; trc <= space_array[user]-19 ; trc++){ // user = 0
                fscanf(fptr,"%s",db[user].tr[trc].note);                   // space_array[user] - 19 = 81
...
void printing_all_data()
{
    for(int user=0 ; user < G_index ; user++){
        printf("%u-%s-%s-%s-%s-%u-%s-%s-%s-%d-%d-%d-%s-%llu-%s-%u-%u-%f-%s",db[user].id,db[user].name,db[user].nrc,db[user].email,db[user].password,
               db[user].phoneNumber,db[user].encryption_key,db[user].recovery_key,db[user].account_status,db[user].account_type,db[user].account_level,
               db[user].minimum_opening_deposit,db[user].currency,db[user].current_amount,db[user].loanStatus,db[user].monthly_income,db[user].loan_amount,
